// Package runner implements `phoned run --id <id>` — the wrapper that
...
// Main is the entry point for `phoned run --id <id>`. It never returns
// except by exiting the process.
func Main(id string) int {
	if !state.ValidID(id) {
		fmt.Fprintf(os.Stderr, "phoned run: invalid app id %q\n", id)
		return 2
	}
	if err := config.EnsureDirs(); err != nil {
		fmt.Fprintf(os.Stderr, "phoned run: %v\n", err)
		return 1
	}
...
func (w *wrapper) runChild(app state.App) (exitCode int, sigName string, err error) {
	fmt.Printf("\x1b[2m[phoned] starting %s: %s\x1b[0m\r\n", app.ID, app.Cmd)
	...
	if err := cmd.Start(); err != nil {
		...
		code := 127
		w.recordEnd(&code, "")
		fmt.Fprintf(os.Stderr, "[phoned] failed to start: %v\r\n", err)
		return 127, "", err
	}
